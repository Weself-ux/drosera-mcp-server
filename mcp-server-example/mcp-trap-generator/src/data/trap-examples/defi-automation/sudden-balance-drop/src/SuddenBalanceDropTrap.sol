...
contract SuddenBalanceDropTrap is ITrap {
    ...
    constructor() {
        ...
        monitoredVaults.push(VaultInfo({
    ...
    function collect() external view override returns (bytes memory) {
        VaultBalance[] memory balances = new VaultBalance[](monitoredVaults.length);
        
        for (uint256 i = 0; i < monitoredVaults.length; i++) {
            VaultInfo memory info = monitoredVaults[i];
            
            uint256 balance = 0;
            try IERC20(info.token).balanceOf(info.vault) returns (uint256 _balance) {
                balance = _balance;
            } catch {}
        ...
        returns (bool shouldTrigger, bytes memory responseData) 
    {
        ...
        for (uint256 i = 0; i < current.length; i++) {
            if (current[i].vault != previous[i].vault || current[i].token != previous[i].token) {
                continue;
            }
        ...
        if (dropCount > 0) {
            BalanceDrop[] memory result = new BalanceDrop[](dropCount);
