...
contract MultiTokenBalanceTrap is ITrap {
    ...
    constructor() {
        ...
        monitoredTokens.push(TokenConfig({
    ...
    function collect() external view override returns (bytes memory) {
        TokenBalance[] memory balances = new TokenBalance[](monitoredTokens.length);
        
        for (uint256 i = 0; i < monitoredTokens.length; i++) {
            TokenConfig memory config = monitoredTokens[i];
            
            uint256 balance = 0;
            try IERC20(config.token).balanceOf(config.account) returns (uint256 _balance) {
                balance = _balance;
            } catch {}
        ...
        returns (bool shouldTrigger, bytes memory responseData) 
    {
        ...
        if (alertCount > 0) {
            BalanceAlert[] memory result = new BalanceAlert[](alertCount);
    ...
    function addMonitoredToken(address token, address account, uint256 minBalance) external {
        monitoredTokens.push(TokenConfig({
            token: token,
            account: account,
            minBalance: minBalance
