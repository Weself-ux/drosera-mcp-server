...
contract SchedulerResponse {
    ...
    event ScheduledActionExecuted(uint256 timestamp, string action, uint256 executionTime);
    ...
    function wasActionExecuted(string calldata action) external view returns (bool) {
        return actionExecuted[action] > 0;
    }

    function getActionExecutionTime(string calldata action) external view returns (uint256) {
